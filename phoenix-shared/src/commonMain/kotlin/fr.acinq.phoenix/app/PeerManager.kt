...
class PeerManager(
    ...
    init {
        launch {
            val wallet = walletManager.walletState.filterNotNull().first()
            val params = walletParamsManager.walletParams.filterNotNull().first()
            _peer.value = buildPeer(wallet, params)
        }
    }

    suspend fun peer() = peerState.filterNotNull().first()
